...
class CourseAgent:
    def __init__(self, config_path: str):
        ...
        self.mcp = MCP()
        self.prompt_template = Path("llm/prompt_templates/course_prompt.txt"
                                    ).read_text(encoding="utf-8")
        
    def answer(self, question: str) -> str:
        query_vector = self.llm_client.get_embedding(question)
        context = "\n".join(self.mcp.query_vector_db(query_vector, domain="course"))
        external = self.mcp.query_external(question, domain="course")
